...
class ApiWorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            try:
                ...
                print(f"API call took: {elapsed_time:.2f} seconds using model: {self._model_id}") # Added model ID here

            if not self._is_running: return

            # Clean reply and parse command AFTER cleaning
            ...
            try:
                ...
                reply_for_command_parsing = cleaned_reply
            ...
            if reply_for_command_parsing and isinstance(reply_for_command_parsing, str):
                try:
                    ...
                    if self._is_running:
                        try:
                            self.cli_error_signal.emit(f"Error parsing command tag: {e}".encode('utf-8'))
            ...
            if command_to_run and self._is_running:
                ...
                try: self._execute_command(command_to_run)
                    ...
                    if self._is_running:
                        try:
                             self.cli_error_signal.emit(f"Error: Unexpected error executing command: {exec_err}".encode('utf-8'))
                        except RuntimeError as sig_e:
    ...
    def _execute_command(self, command):
        ...
        # ============================================================= #
        # <<< MODIFIED: Echo AI command with CWD >>>
        # Removed the 'exec_prefix' as it seems confusing here.
        # The echo now includes the CWD the command is intended for.
        # ============================================================= #
        status_message = f"Model {self._cwd}: {command}"
        if self._is_running:
            try:
                # Emit the command echo with CWD as utf-8 bytes
                self.cli_output_signal.emit(status_message.encode('utf-8'))
            except RuntimeError as e:
                print(f"Warning: Could not emit AI command echo signal: {e}")
                return # Don't proceed if echo fails
        # ============================================================= #
        # <<< END MODIFICATION >>>
        # ============================================================= #
        ...
        if command.strip().lower().startswith('cd '):
            ...
            try:
                ...
                if os.path.isdir(target_dir):
                    ...
                    if self._is_running:
                        try: self.cli_error_signal.emit(error_msg.encode('utf-8'))
            ...
            try:
                ...
                os_name = platform.system() # Get OS name again
                if os_name == "Windows":
                    try:
                ...
                if result.returncode != 0 and self._is_running:
                     ...
                     if not emitted_stderr_meaningful or str(result.returncode) not in stderr_str_for_check:
                         ...
                         try: self.cli_error_signal.emit(exit_msg.encode('utf-8'))
                ...
                if self._is_running:
                    try: self.cli_error_signal.emit(timeout_msg.encode('utf-8'))
                ...
                if self._is_running:
                    try: self.cli_error_signal.emit(fnf_msg.encode('utf-8'))
                ...
                traceback.print_exc()
                if self._is_running:
                    try: self.cli_error_signal.emit(exec_err_msg.encode('utf-8'))
    ...
    def run(self):
        ...
        try:
            ...
            if command.strip().lower().startswith('cd '):
                ...
                try:
                    ...
                    if os.path.isdir(target_dir):
                        self._cwd = target_dir
                        if self._is_running:
                            try: self.directory_changed_signal.emit(self._cwd, True)
                            except RuntimeError as e: print(f"Warning: Could not emit directory changed signal: {e}")
                    else:
                        error_msg = f"Error: Directory not found: '{target_dir}' (Resolved from '{path_part}')"
                        if self._is_running:
            ...
            if os_name == "Windows":
                ...
                shell_path = os.environ.get("SHELL", "/bin/sh")
                run_args = [shell_path, "-c", command]; use_shell = False; creationflags = 0; preexec_fn = os.setsid
            if run_args is None: return
            ...
            try:
                 ...
                 traceback.print_exc()
                 if self._is_running:
                     try: self.cli_error_signal.emit(popen_err_msg.encode('utf-8'))
            ...
            if self._process:
                ...
                if process_ref:
                    try:
                         while process_ref.poll() is None and self._is_running: QThread.msleep(50)
                         if self._is_running: process_return_code = process_ref.poll()
                ...
                if stdout_thread: stdout_thread.quit(); stdout_thread.wait(200)
            ...
            traceback.print_exc()
            if self._is_running:
                 try: self.cli_error_signal.emit(f"Unexpected thread error: {run_err}".encode('utf-8'))
